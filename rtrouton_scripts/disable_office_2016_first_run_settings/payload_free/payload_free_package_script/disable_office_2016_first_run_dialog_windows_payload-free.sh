...
DisableOffice2016FirstRun(){
   ...
   /usr/bin/defaults write "$3"/Library/Preferences/com.microsoft."$app" kSubUIAppCompletedFirstRunSetup1507 -bool true
   /usr/bin/defaults write "$3"/Library/Preferences/com.microsoft."$app" SendAllTelemetryEnabled -bool "$submit_diagnostic_data_to_microsoft"

   # Outlook and OneNote require one additional first run setting to be disabled

   if [[ $app == "Outlook" ]] || [[ $app == "onenote.mac" ]]; then

     /usr/bin/defaults write "$3"/Library/Preferences/com.microsoft."$app" FirstRunExperienceCompletedO15 -bool true
...
if [[ -e "$3/Applications/Microsoft Excel.app" ]]; then
app=Excel
...
if [[ -e "$3/Applications/Microsoft OneNote.app" ]]; then
app=onenote.mac
...
if [[ -e "$3/Applications/Microsoft Outlook.app" ]]; then
app=Outlook
...
if [[ -e "$3/Applications/Microsoft PowerPoint.app" ]]; then
app=Powerpoint
...
if [[ -e "$3/Applications/Microsoft Word.app" ]]; then
app=Word
