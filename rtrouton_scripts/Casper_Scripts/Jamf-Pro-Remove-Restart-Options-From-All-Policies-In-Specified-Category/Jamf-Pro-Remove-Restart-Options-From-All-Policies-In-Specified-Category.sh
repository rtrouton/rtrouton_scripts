...
RemoveRestartFromComputerPolicy(){

curloutput=$(mktemp)

if [[ -z "$NoBearerToken" ]]; then
	CheckAndRenewAPIToken
	/usr/bin/curl --write-out '%{http_code}\t' --silent --output "$curloutput" --header "Authorization: Bearer ${api_token}" -H "Content-Type: application/xml" "${jamfpro_url}/JSSResource/policies/id/${policyID}" -X PUT -d '<policy><reboot><no_user_logged_in>Do not restart</no_user_logged_in><user_logged_in>Do not restart</user_logged_in></reboot></policy>'
else
	/usr/bin/curl --write-out '%{http_code}\t' --silent --output "$curloutput" --user "${jamfpro_user}:${jamfpro_password}" -H "Content-Type: application/xml" "${jamfpro_url}/JSSResource/policies/id/${policyID}" -X PUT -d '<policy><reboot><no_user_logged_in>Do not restart</no_user_logged_in><user_logged_in>Do not restart</user_logged_in></reboot></policy>'
fi
