...
# and uses that information to generate a report about the mobile devices
...
if [[ -z "$NoBearerToken" ]]; then
	CheckAndRenewAPIToken
    /usr/bin/curl -sf --header "Authorization: Bearer ${api_token}" "${jamfpro_url}/JSSResource/mobiledevices/match/${UserToMatch}" -H "Accept: application/xml" | xmllint --format - >> "$xmltempfile"
else
    /usr/bin/curl -sfu "$jamfpro_user:$jamfpro_password" "${jamfpro_url}/JSSResource/mobiledevices/match/${UserToMatch}" -H "Accept: application/xml" | xmllint --format - >> "$xmltempfile"
fi

done < "$filename"

# Extract the Jamf Pro mobile device IDs
...
while read -r ID; do
	...
	if [[ "$ID" =~ ^[0-9]+$ ]]; then
		if [[ -z "$NoBearerToken" ]]; then
			CheckAndRenewAPIToken
		    MobileDeviceRecord=$(/usr/bin/curl -sf --header "Authorization: Bearer ${api_token}" "${jamfpro_url}/JSSResource/mobiledevices/id/$ID" -H "Accept: application/xml" 2>/dev/null)
		else
		    MobileDeviceRecord=$(/usr/bin/curl -sfu "$jamfpro_user:$jamfpro_password" "${jamfpro_url}/JSSResource/mobiledevices/id/$ID" -H "Accept: application/xml" 2>/dev/null)
		fi
			if [[ ! -f "$report_file" ]]; then
				/usr/bin/touch "$report_file"
				printf "Jamf Pro ID Number\tAssigned User\tAssigned User Email\tModel\tSerial Number\tUDID\tJamf Pro URL\n" > "$report_file"
			fi

			AssignedUser=$(echo "$MobileDeviceRecord" | xmllint --xpath '//mobile_device/location/username/text()' - 2>/dev/null)
			AssignedUserEmail=$(echo "$MobileDeviceRecord" | xmllint --xpath '//mobile_device/location/email_address/text()' - 2>/dev/null)
			MachineModel=$(echo "$MobileDeviceRecord" | xmllint --xpath '//mobile_device/general/model/text()' - 2>/dev/null)
			SerialNumber=$(echo "$MobileDeviceRecord" | xmllint --xpath '//mobile_device/general/serial_number/text()' - 2>/dev/null)
			JamfProID=$(echo "$MobileDeviceRecord" | xmllint --xpath '//mobile_device/general/id/text()' - 2>/dev/null)
			UDIDIdentifier=$(echo "$MobileDeviceRecord" | xmllint --xpath '//mobile_device/general/udid/text()' - 2>/dev/null)						
			JamfProURL=$(echo "$jamfpro_url"/mobileDevices.html?id="$JamfProID")
			
			if [[ $? -eq 0 ]]; then
				printf "$JamfProID\t$AssignedUser\t$AssignedUserEmail\t$MachineModel\t$SerialNumber\t$UDIDIdentifier\t${JamfProURL}\n" >> "$report_file"
			else
				echo "ERROR! Failed to read mobile device record with id $JamfProID"
				error=1
			fi
	fi
