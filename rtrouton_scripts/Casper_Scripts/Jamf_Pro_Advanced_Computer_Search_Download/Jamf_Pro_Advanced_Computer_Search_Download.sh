...
DownloadAdvancedComputerSearch(){
	...
	if [[ -z "$NoBearerToken" ]]; then
		CheckAndRenewAPIToken
		FormattedAdvancedComputerSearch=$(/usr/bin/curl -s --header "Authorization: Bearer ${api_token}" -H "Accept: application/xml" "${jamfpro_url}/JSSResource/advancedcomputersearches/id/$1" -X GET | tr $'\n' $'\t' | sed -E 's|<computers>.*</computers>||' |  tr $'\t' $'\n' | xmllint --format - )
	else
		FormattedAdvancedComputerSearch=$(/usr/bin/curl -su "${jamfpro_user}:${jamfpro_password}" -H "Accept: application/xml" "${jamfpro_url}/JSSResource/advancedcomputersearches/id/$1" -X GET | tr $'\n' $'\t' | sed -E 's|<computers>.*</computers>||' |  tr $'\t' $'\n' | xmllint --format - )
	fi

	if [[ -n "$FormattedAdvancedComputerSearch" ]]; then
	
		# Identify and display the advanced computer search's name.
		DisplayName=$(echo "$FormattedAdvancedComputerSearch" | xmllint --xpath "/advanced_computer_search/name/text()" - 2>/dev/null | sed -e 's|:|(colon)|g' -e 's/\//\\/g')
...
if [[ $ERROR -eq 0 ]]; then
	...
	if [[ -z "$NoBearerToken" ]]; then
		CheckAndRenewAPIToken
		AdvancedComputerSearch_id_list=$(/usr/bin/curl -s --header "Authorization: Bearer ${api_token}" -H "Accept: application/xml" "${jamfpro_url}/JSSResource/advancedcomputersearches" | xmllint --xpath "//id" - 2>/dev/null)
	else
		AdvancedComputerSearch_id_list=$(/usr/bin/curl -su "${jamfpro_user}:${jamfpro_password}" -H "Accept: application/xml" "${jamfpro_url}/JSSResource/advancedcomputersearches" | xmllint --xpath "//id" - 2>/dev/null)
	fi
