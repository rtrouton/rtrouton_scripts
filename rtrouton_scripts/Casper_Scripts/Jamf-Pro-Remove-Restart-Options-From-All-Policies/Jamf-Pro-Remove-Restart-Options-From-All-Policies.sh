...
curl --write-out '%{http_code}\t' --silent --output "$curloutput" -H "Content-Type: application/xml" -X PUT -d "<policy><reboot><no_user_logged_in>Do not restart</no_user_logged_in><user_logged_in>Do not restart</user_logged_in></reboot></policy>" "$jamfpro_url"/JSSResource/policies/id/"$ID" --user "$jamfpro_user:$jamfpro_password"
