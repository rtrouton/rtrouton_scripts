...
# This script is designed to clear failed MDM commands from a Jamf Pro smart or static computer group. 
...
# If you choose to hardcode the Jamf Pro ID of the smart or static computer group whose members you want to clear
# failed MDM commands from, please set it using the groupID variable below.
#
# For example, if the Jamf Pro ID of the computer group in question is 1, you would set the groupID variable as
# shown below:
#
# groupID="1"
#
...
# If the Jamf Pro ID of the smart or static computer group whose members you want to clear failed 
# MDM commands from has not been provided, you will be prompted to enter the Jamf Pro ID of the smart
# or static computer group.

if [[ -z "$groupID" ]]; then
     echo ""
     echo ""
     echo "The smart or static computer group you want to clear failed MDM commands from has not been specified."
     echo ""
     read -p "Please enter the Jamf Pro ID of the smart or static computer group : " groupID
fi
...
# This function uses the Jamf Pro Classic API to get the display name of a specified Jamf Pro smart or static computer group.

GetGroupDisplayName () {

groupName=$(/usr/bin/curl -sf --header "Authorization: Bearer ${api_token}" -H "Accept: application/xml" "${jamfpro_url}/JSSResource/computergroups/id/${groupID}" | xmllint --xpath '//computer_group/name/text()' - 2>/dev/null)

}

# This function uses the Jamf Pro Classic API to clear all failed MDM commands associated with the
# members of a specified Jamf Pro smart or static computer group.

ClearFailedMDMCommandsFromGroup () {

/usr/bin/curl -sf --header "Authorization: Bearer ${api_token}" "${jamfpro_url}/JSSResource/commandflush/computergroups/id/${groupID}/status/Failed" -X DELETE

}

# Get a Jamf Pro API token, then use the token to authenticate clearing all failed MDM commands
# associated with the members of the smart or static computer group specified by the "groupID"
...
if [[ "$groupID" =~ ^[0-9]+$ ]]; then
    ...
    # is not used by a current smart or static computer group.
    
    if [[ -n ${groupName} ]]; then

         # If the group name lookup has succeeded, clear failed MDM commands from the
         # specified Jamf Pro smart or static computer group.
         ...
         echo "ERROR: Jamf Pro ID $groupID is not used by a current smart or static computer group."
         error=1
    fi
else
   echo "ERROR: $groupID is not a valid Jamf Pro ID number."
   echo "Please verify value entered for the Jamf Pro ID of the smart or static computer group."
   error=1
fi
