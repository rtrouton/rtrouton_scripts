...
function checkJava()	{
	...
	# Java isn't installed, install OpenJDK 11.x 
	...
	if [[ $? -eq 0 ]]; then
		...
		echo "Java not installed. Installing OpenJDK 11.x."
		/usr/bin/yum -y install java-11-openjdk-devel
		echo ""
		echo "Installed Java:"
		echo "$(java -version)"
	fi
...
# Check to see if Java is already installed and install OpenJDK 11.x
