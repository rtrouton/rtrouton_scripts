...
while read -r ID; do
	...
	if [[ "$ID" =~ ^[0-9]+$ ]]; then
		if [[ -z "$NoBearerToken" ]]; then
			...
			JamfProURL=$(echo "$jamfpro_url"/mobileDevices.html?id="$ID")
			
			if [[ $? -eq 0 ]]; then
				printf "$ID\t$MachineModel\t$SerialNumber\t$UDIDIdentifier\t${JamfProURL}\n" >> "$report_file"
			else
				echo "ERROR! Failed to read mobile device record with id $ID"
			fi
	fi
