...
while read -r SerialNumber; do
		...
		if [[ -z "$NoBearerToken" ]]; then
			CheckAndRenewAPIToken
		    MobileDeviceRecord=$(/usr/bin/curl -sf --header "Authorization: Bearer ${api_token}" "${jamfpro_url}/JSSResource/mobiledevices/serialnumber/$SerialNumber" -H "Accept: application/xml" 2>/dev/null)
		else
		    MobileDeviceRecord=$(/usr/bin/curl -sfu "$jamfpro_user:$jamfpro_password" "${jamfpro_url}/JSSResource/mobiledevices/serialnumber/$SerialNumber" -H "Accept: application/xml" 2>/dev/null)
		fi
			if [[ ! -f "$report_file" ]]; then
				touch "$report_file"
				printf "Jamf Pro ID Number\tModel\tSerial Number\tUDID\tJamf Pro URL\n" > "$report_file"
			fi
			...
			if [[ $? -eq 0 ]]; then
				printf "$JamfProID\t$MachineModel\t$SerialNumber\t$UDIDIdentifier\t${JamfProURL}\n" >> "$report_file"
			else
				echo "ERROR! Failed to read mobile device record with serial number $SerialNumber"
			fi
