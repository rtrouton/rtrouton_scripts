...
if [[ -n $filename && -r $filename ]]; then
	...
	# Set up the Jamf Pro Packages ID URL
	...
	do
		...
		if [[ "$PackagesID" =~ ^[0-9]+$ ]]; then
		  ...
		  if [[ -z "$NoBearerToken" ]]; then
		  	  CheckAndRenewAPIToken
		  	  PackagesName=$(/usr/bin/curl -s --header "Authorization: Bearer ${api_token}" -H "Accept: application/xml" "${jamfproIDURL}/$PackagesID" | xmllint --xpath '//package/name/text()' - 2>/dev/null)  
		  else
		  	  PackagesName=$(/usr/bin/curl -su "${jamfpro_user}:${jamfpro_password}" -H "Accept: application/xml" "${jamfproIDURL}/$PackagesID" | xmllint --xpath '//package/name/text()' - 2>/dev/null)
		  fi
		  ...
		  #if [[ -z "$NoBearerToken" ]]; then
		  #	  CheckAndRenewAPIToken
		  #    /usr/bin/curl -s --header "Authorization: Bearer ${api_token}" "${jamfproIDURL}/$PackagesID" -X DELETE
		  #else
