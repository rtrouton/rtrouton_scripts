...
PLIST="$HOME/Library/Preferences/com.github.jamfpro-info1.plist"
...
if [[ -z "$NoBearerToken" ]]; then
	CheckAndRenewAPIToken
	/usr/bin/curl --write-out '%{http_code}\t' --silent --output "$curloutput" --header "Authorization: Bearer ${api_token}" -H "Content-Type: application/xml" "${jamfpro_url}/JSSResource/policies/id/${ID}" -X PUT -d '<policy><general><enabled>false</enabled></general></policy>'
else
	/usr/bin/curl --write-out '%{http_code}\t' --silent --output "$curloutput" --user "${jamfpro_user}:${jamfpro_password}" -H "Content-Type: application/xml" "${jamfpro_url}/JSSResource/policies/id/${ID}" -X PUT -d '<policy><general><enabled>false</enabled></general></policy>'
fi
