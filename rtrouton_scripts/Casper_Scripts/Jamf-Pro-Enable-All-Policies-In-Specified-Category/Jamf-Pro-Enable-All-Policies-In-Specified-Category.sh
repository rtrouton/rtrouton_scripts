...
# This script enables all policies in a specified category.
...
echo -e "\nThis script enables all Jamf Pro policies in a specified category.\n"
...
if [[ -z "$NoBearerToken" ]]; then
	CheckAndRenewAPIToken
	/usr/bin/curl --write-out '%{http_code}\t' --silent --output "$curloutput" --header "Authorization: Bearer ${api_token}" -H "Content-Type: application/xml" "${jamfpro_url}/JSSResource/policies/id/${policyID}" -X PUT -d '<policy><general><enabled>true</enabled></general></policy>'
else
	/usr/bin/curl --write-out '%{http_code}\t' --silent --output "$curloutput" --user "${jamfpro_user}:${jamfpro_password}" -H "Content-Type: application/xml" "${jamfpro_url}/JSSResource/policies/id/${policyID}" -X PUT -d '<policy><general><enabled>true</enabled></general></policy>'
fi
...
echo -e "on $jamfpro_url are going to be enabled:\n"
echo "$(echo "$FormattedCategoryXML" | awk -F "[><]" '/name/{print $3}')"

echo -e "\nAre you absolutely certain you want to enable "
...
for policyID in ${PoliciesInCategory_id}; do
    echo -e "\nEnabling policy ID $policyID ..."
    EnableComputerPolicy

    if [[ $? -eq 0 ]]; then
	    echo -e "\nEnabled policy ID $policyID."
    else
	    echo -e "\nERROR! Failed to enable policy ID $policyID."
    fi
done
