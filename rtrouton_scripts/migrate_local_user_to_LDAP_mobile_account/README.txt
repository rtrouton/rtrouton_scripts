...
The main thing you should need to edit is the "Verify NetID", as that does a lookup for an LDAP group. To fix it, you'll need to change the "lookupAccount" value to a group on your AD domain (preferably, a group that nobody should be deleting anytime soon) and do some minor text editing. You may also want to add "| grep -v your_local_admin_account_shortname |" to the listUsers value, as that will prevent your local admin account from showing up as one that can be migrated.
